	...
	function pollResults () {
		...
		async.waterfall([
				...
				function (done) {
					...
					// if this is the first run of the agent, grab the last known
					// anchor with results
					if (status.schedule.totalRunCount === 1) {
						var start = new Date();

						services.threeTaps.poll({
							location : pollingOptions.location
						}, function (err, result) {
							if (err) {
								return done(err);
							}
							...
							app.log.info(
								'generated anchor %s from initial poll in %s',
								result.anchor,
								countdown(start, new Date(), countdown.MILLISECONDS));

							return done(null, result.anchor);
						});
					}
