...
module.exports = function (app, self) {
	...
	self.getLatest = function (tryCount, callback) {
		if (typeof callback === 'undefined' && typeof tryCount === 'function') {
			callback = tryCount;
			tryCount = config.maxRetryCount;
		}
	...
	self.upsertPostings = function (postings, tryCount, callback) {
		...
		var postingsToUpsert = [];

		async.whilst(
			function () {
				postingsToUpsert = postings.splice(0, config.maxBulkUpsertCount);
				return postingsToUpsert.length;
			},
			function (proceed) {
				request({
					...
					// check for retry
					if (!res || (res && res.statusCode >= 500)) {
						if (config.maxRetryCount && tryCount < config.maxRetryCount) {
							tryCount++;
							return self.upsertPostings(postings, tryCount, callback);
						}

						return proceed(err || json || new Error('no response from server'));
					}
