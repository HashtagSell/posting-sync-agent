...
module.exports = function (app, self) {
	...
	self.getLatest = function (options, tryCount, callback) {
		...
		if (!options.state && !options.metro) {
			return setImmediate(
				callback,
				new Error('metro or state is required to get latest posting'));
		}
		...
		request({
			...
			if (err) {
				err.url = [config.url, 'latest'].join('/');
	...
	self.upsertPostings = function (postings, callback) {
		...
		async.whilst(
			...
			function (next) {
				...
				var
					postingsToUpsert = postings.slice(
						startIndex,
						startIndex + config.maxBulkUpsertCount),
					upsertStart = new Date();
				...
				request({
					...
					app.log.trace(
						'completed upsert of postings %d to %d of %d in %s',
						startIndex,
						(startIndex + config.maxBulkUpsertCount),
						postings.length,
						countdown(upsertStart, new Date(), countdown.MILLISECONDS));
